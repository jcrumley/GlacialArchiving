 ...
 */		
	$.navAsAjax = false; // Your left nav in your app will no longer fire ajax calls  //jc turned this off
...
function setMenuActive() {
    ...
    for (index = 0; index < elements.length; ++index) {
        ...
        if (typeof target === 'undefined' || target == null) continue;
    
        var target = target.replace(/\?/g, "\\?").replace(/\//g, "\\/").replace(/\=/g, "\\=");

        if (pathname.match(target) != null) {
            $(t).parent().addClass("active");
            //open active level
            $(t).parents("ul").slideDown(2);
            $(t).parents("ul").parent("li").find("b:first").html('<em class="fa fa-collapse-o"></em>');
            $(t).parents("ul").parent("li").addClass("open");
            break;
        }
    }
}
...
function checkURL() {
	...
	//if (url) {
	if (url && url != "undefined") //jc added undefined
    {
