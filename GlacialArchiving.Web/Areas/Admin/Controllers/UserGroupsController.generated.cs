...
{   
    /// <summary>
    ///     The definition of UserGroupsController
    /// </summary>
    public partial class UserGroupsController : SecureController
    {
        /// <summary>
        ///     Default constructor for UserGroupsController
        /// </summary>
        ...
        partial void BeforeValidate(UserGroup obj);
        partial void BeforeCreate(UserGroup obj);
        partial void AfterCreate(UserGroup obj);
        partial void BeforeEdit(UserGroup oldObj, UserGroup obj);
        partial void AfterEdit(UserGroup obj);
        ...
        /// <summary>
        ///     Sends the user to the listing page for UserGroups
        /// </summary>
        ...
        public ActionResult Index(LoggedInUser currentUser)
        {
            ...
            ViewResult result = View(work.UserGroupRepository.AllIncluding(obj => obj.CreatedBy, obj => obj.ModifiedBy, obj => obj.Memberships));
        ...
        /// <summary>
        ///     View to create a new UserGroup in the system
        /// </summary>
        /// <param name="currentUser">The current logged in user</param>
        /// <returns></returns>
        ...
        /// <summary>
        ///     Submission of a UserGroup from the UI
        /// </summary>
        /// <param name="obj">The new object to insert into the database</param>
        /// <param name="currentUser">The current logged in user</param>
        /// <returns></returns>
        [HttpPost]
        public ActionResult Create(UserGroup obj, LoggedInUser currentUser)
        {
            this.log.Info("Create [POST]", "Start of function");
            if (obj.Id == 0)
                ModelState["UserGroupId"].Errors.Clear();
                
            this.BeforeValidate(obj);
            if (ModelState.IsValid) 
            {
                this.BeforeCreate(obj);
                work.UserGroupRepository.InsertOrUpdate(obj, currentUser.User);
                work.Save();
        ...
        /// <summary>
        ///     View to edit an existing UserGroup record in the system
        /// </summary>
        ...
        public ActionResult Edit(int id, LoggedInUser currentUser)
        {
            ...
            ViewResult result = View(work.UserGroupRepository.Find(id));
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Edit(UserGroup obj, LoggedInUser currentUser)
        {
            ...
            if (ModelState.IsValid) 
            {
                UserGroup oldObj = work.UserGroupRepository.Find(obj.Id);
                obj.SyncExcludedFields(oldObj);
            
                this.BeforeEdit(oldObj, obj);
                work.UserGroupRepository.InsertOrUpdate(obj, currentUser.User);
                work.Save();
                obj = work.UserGroupRepository.Find(obj.Id); //need to rehydrate the FKs
                this.AfterEdit(obj);
        ...
        public ActionResult DeleteConfirmed(int id, LoggedInUser currentUser)
        {
            ...
            work.UserGroupRepository.Delete(id);
            work.Save();
            this.AfterDelete(id);
            this.log.Info("Delete [POST]", "Object has been deleted");
            return RedirectToAction("Index");
        }  // POST: /Admin/UserGroups/Delete/{id}
        #endregion
        #region Section 'Actions - Inline'
        /// <summary>
        ///     Retrieve view to create/edit an existing UserGroup record in the system
        /// </summary>
        /// <param name="id">The id of the record to edit</param>
        /// <param name="currentUser">The current logged in user</param>
        /// <returns></returns>
        ...
        /// <returns></returns>
        [HttpPost]
        public MvcHtmlString InsertOrUpdate(UserGroup obj, LoggedInUser currentUser)
        {
            this.log.Info("Edit [POST]", "Start of function");
            if (obj.Id == 0)
                ModelState["UserGroupId"].Errors.Clear();
        ...
        {
            this.log.Info("Delete", "Start of function");
            ViewResult result = View("_Display", work.UserGroupRepository.Find(id));
            this.log.Info("Delete", "View created and returning");
            return result;
