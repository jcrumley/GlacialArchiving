...
{   
    /// <summary>
    ///     The definition of fredsController
    /// </summary>
    public partial class fredsController : SecureController
    {
        /// <summary>
        ///     Default constructor for fredsController
        /// </summary>
        public fredsController() 
        { 
        ...
        partial void BeforeValidate(fred obj);
        partial void BeforeCreate(fred obj);
        partial void AfterCreate(fred obj);
        partial void BeforeEdit(fred oldObj, fred obj);
        partial void AfterEdit(fred obj);
        ...
        /// <summary>
        ///     Sends the user to the listing page for freds
        /// </summary>
        ...
        public ActionResult Index(LoggedInUser currentUser)
        {
            ...
            ViewResult result = View(work.fredRepository.AllIncluding(obj => obj.CreatedBy, obj => obj.ModifiedBy));
        ...
        /// <summary>
        ///     View to create a new fred in the system
        /// </summary>
        /// <param name="currentUser">The current logged in user</param>
        /// <returns></returns>
        public ActionResult Create(LoggedInUser currentUser)
        {
            ...
            return result;
        }   // GET: /Admin/freds/Create
        /// <summary>
        ///     Submission of a fred from the UI
        /// </summary>
        /// <param name="obj">The new object to insert into the database</param>
        /// <param name="currentUser">The current logged in user</param>
        /// <returns></returns>
        [HttpPost]
        public ActionResult Create(fred obj, LoggedInUser currentUser)
        {
            this.log.Info("Create [POST]", "Start of function");
            if (obj.Id == 0)
                ModelState["fredId"].Errors.Clear();
                
            this.BeforeValidate(obj);
            if (ModelState.IsValid) 
            {
                this.BeforeCreate(obj);
                work.fredRepository.InsertOrUpdate(obj, currentUser.User);
                work.Save();
        ...
        }  // POST: /Admin/freds/Create
        #endregion
        #region Section 'Actions - Edit'
        /// <summary>
        ///     View to edit an existing fred record in the system
        /// </summary>
        ...
        public ActionResult Edit(int id, LoggedInUser currentUser)
        {
            ...
            ViewResult result = View(work.fredRepository.Find(id));
            this.log.Info("Edit", "View created and returning");
            return result;
        }  // GET: /Admin/freds/Edit/{id}
        /// <summary>
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Edit(fred obj, LoggedInUser currentUser)
        {
            ...
            if (ModelState.IsValid) 
            {
                fred oldObj = work.fredRepository.Find(obj.Id);
                obj.SyncExcludedFields(oldObj);
            
                this.BeforeEdit(oldObj, obj);
                work.fredRepository.InsertOrUpdate(obj, currentUser.User);
                work.Save();
                obj = work.fredRepository.Find(obj.Id); //need to rehydrate the FKs
                this.AfterEdit(obj);
        ...
        }  // POST: /Admin/freds/Delete/{id}
        #endregion
        #region Section 'Actions - Inline'
        /// <summary>
        ///     Retrieve view to create/edit an existing fred record in the system
        /// </summary>
        /// <param name="id">The id of the record to edit</param>
        /// <param name="currentUser">The current logged in user</param>
        /// <returns></returns>
        ...
        /// <returns></returns>
        [HttpPost]
        public MvcHtmlString InsertOrUpdate(fred obj, LoggedInUser currentUser)
        {
            this.log.Info("Edit [POST]", "Start of function");
            if (obj.Id == 0)
                ModelState["fredId"].Errors.Clear();
        ...
        {
            this.log.Info("Delete", "Start of function");
            ViewResult result = View("_Display", work.fredRepository.Find(id));
            this.log.Info("Delete", "View created and returning");
            return result;
