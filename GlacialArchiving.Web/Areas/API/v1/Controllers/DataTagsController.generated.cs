...
{
    /// <summary>
    ///     Creates the interface via WebAPI for DataTags
    /// </summary>
    public partial class DataTagsController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of DataTags in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all DataTags in the system that the logged in user can see</returns>
        public IEnumerable<DataTag> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<DataTag> collection = work.DataTagRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<DataTag> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the DataTag record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public DataTag Get(int id, [ModelBinder] LoggedInUser user)
        {
        ...
        partial void OnGetById(DataTag obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a DataTag record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] DataTag obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.DataTagRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(DataTag obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(DataTag obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a DataTag  record from the system
        /// </summary>
        ...
        public HttpResponseMessage Delete(int id, [ModelBinder] LoggedInUser user)
        {
            ...
            try
            {
                ...
                work.DataTagRepository.Delete(id);
                work.Save();
