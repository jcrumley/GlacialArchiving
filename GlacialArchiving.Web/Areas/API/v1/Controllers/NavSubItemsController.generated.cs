...
{
    /// <summary>
    ///     Creates the interface via WebAPI for NavSubItems
    /// </summary>
    public partial class NavSubItemsController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of NavSubItems in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all NavSubItems in the system that the logged in user can see</returns>
        public IEnumerable<NavSubItem> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<NavSubItem> collection = work.NavSubItemRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<NavSubItem> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the NavSubItem record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public NavSubItem Get(int id, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetById", "Start of function");
            NavSubItem obj = work.NavSubItemRepository.Find(id);
            this.OnGetById(obj, user);
            this.log.Info("GetById", "Returning data");
            return obj;
        }
        partial void OnGetById(NavSubItem obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a NavSubItem record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] NavSubItem obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.NavSubItemRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(NavSubItem obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(NavSubItem obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a NavSubItem  record from the system
        /// </summary>
