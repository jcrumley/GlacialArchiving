...
{
    /// <summary>
    ///     Creates the interface via WebAPI for Settings
    /// </summary>
    public partial class SettingsController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of Settings in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all Settings in the system that the logged in user can see</returns>
        public IEnumerable<Setting> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<Setting> collection = work.SettingRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<Setting> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the Setting record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public Setting Get(int id, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetById", "Start of function");
            Setting obj = work.SettingRepository.Find(id);
            this.OnGetById(obj, user);
            this.log.Info("GetById", "Returning data");
            return obj;
        }
        partial void OnGetById(Setting obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a Setting record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] Setting obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.SettingRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(Setting obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(Setting obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a Setting  record from the system
        /// </summary>
