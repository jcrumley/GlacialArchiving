...
{
    /// <summary>
    ///     Creates the interface via WebAPI for ValidationErrors
    /// </summary>
    public partial class ValidationErrorsController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of ValidationErrors in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all ValidationErrors in the system that the logged in user can see</returns>
        public IEnumerable<ValidationError> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<ValidationError> collection = work.ValidationErrorRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<ValidationError> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the ValidationError record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public ValidationError Get(int id, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetById", "Start of function");
            ValidationError obj = work.ValidationErrorRepository.Find(id);
            this.OnGetById(obj, user);
            this.log.Info("GetById", "Returning data");
            return obj;
        }
        partial void OnGetById(ValidationError obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a ValidationError record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] ValidationError obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.ValidationErrorRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(ValidationError obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(ValidationError obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a ValidationError  record from the system
        /// </summary>
