...
{
    /// <summary>
    ///     Creates the interface via WebAPI for SimpleHTMLPages
    /// </summary>
    public partial class SimpleHTMLPagesController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of SimpleHTMLPages in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all SimpleHTMLPages in the system that the logged in user can see</returns>
        public IEnumerable<SimpleHTMLPage> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<SimpleHTMLPage> collection = work.SimpleHTMLPageRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<SimpleHTMLPage> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the SimpleHTMLPage record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public SimpleHTMLPage Get(int id, [ModelBinder] LoggedInUser user)
        {
        ...
        partial void OnGetById(SimpleHTMLPage obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a SimpleHTMLPage record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] SimpleHTMLPage obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.SimpleHTMLPageRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(SimpleHTMLPage obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(SimpleHTMLPage obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a SimpleHTMLPage  record from the system
        /// </summary>
