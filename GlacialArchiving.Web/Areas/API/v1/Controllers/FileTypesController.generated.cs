...
{
    /// <summary>
    ///     Creates the interface via WebAPI for FileTypes
    /// </summary>
    public partial class FileTypesController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of FileTypes in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all FileTypes in the system that the logged in user can see</returns>
        public IEnumerable<FileType> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<FileType> collection = work.FileTypeRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<FileType> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the FileType record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public FileType Get(int id, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetById", "Start of function");
            FileType obj = work.FileTypeRepository.Find(id);
            this.OnGetById(obj, user);
            this.log.Info("GetById", "Returning data");
            return obj;
        }
        partial void OnGetById(FileType obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a FileType record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] FileType obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.FileTypeRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(FileType obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(FileType obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a FileType  record from the system
        /// </summary>
