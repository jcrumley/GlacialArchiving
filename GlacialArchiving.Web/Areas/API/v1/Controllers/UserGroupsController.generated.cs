...
{
    /// <summary>
    ///     Creates the interface via WebAPI for UserGroups
    /// </summary>
    public partial class UserGroupsController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of UserGroups in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all UserGroups in the system that the logged in user can see</returns>
        public IEnumerable<UserGroup> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<UserGroup> collection = work.UserGroupRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<UserGroup> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the UserGroup record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public UserGroup Get(int id, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetById", "Start of function");
            UserGroup obj = work.UserGroupRepository.Find(id);
            this.OnGetById(obj, user);
            this.log.Info("GetById", "Returning data");
            return obj;
        }
        partial void OnGetById(UserGroup obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a UserGroup record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] UserGroup obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.UserGroupRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(UserGroup obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(UserGroup obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a UserGroup  record from the system
        /// </summary>
