...
{
    /// <summary>
    ///     Creates the interface via WebAPI for UploadedItems
    /// </summary>
    public partial class UploadedItemsController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of UploadedItems in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all UploadedItems in the system that the logged in user can see</returns>
        public IEnumerable<UploadedItem> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<UploadedItem> collection = work.UploadedItemRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<UploadedItem> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the UploadedItem record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public UploadedItem Get(int id, [ModelBinder] LoggedInUser user)
        {
        ...
        partial void OnGetById(UploadedItem obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a UploadedItem record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] UploadedItem obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.UploadedItemRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(UploadedItem obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(UploadedItem obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a UploadedItem  record from the system
        /// </summary>
        ...
        public HttpResponseMessage Delete(int id, [ModelBinder] LoggedInUser user)
        {
            ...
            try
            {
                ...
                work.UploadedItemRepository.Delete(id);
                work.Save();
