...
{
    /// <summary>
    ///     Creates the interface via WebAPI for Users
    /// </summary>
    public partial class UsersController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of Users in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all Users in the system that the logged in user can see</returns>
        public IEnumerable<User> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<User> collection = work.UserRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<User> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the User record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public User Get(int id, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetById", "Start of function");
            User obj = work.UserRepository.Find(id);
            this.OnGetById(obj, user);
            this.log.Info("GetById", "Returning data");
            return obj;
        }
        partial void OnGetById(User obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a User record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] User obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.UserRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(User obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(User obj, LoggedInUser user);
