...
{
    /// <summary>
    ///     Creates the interface via WebAPI for FileData
    /// </summary>
    public partial class FileDataController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of FileData in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all FileData in the system that the logged in user can see</returns>
        public IEnumerable<FileData> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<FileData> collection = work.FileDataRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<FileData> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the FileData record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public FileData Get(int id, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetById", "Start of function");
            FileData obj = work.FileDataRepository.Find(id);
            this.OnGetById(obj, user);
            this.log.Info("GetById", "Returning data");
            return obj;
        }
        partial void OnGetById(FileData obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a FileData record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] FileData obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.FileDataRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(FileData obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(FileData obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a FileData  record from the system
        /// </summary>
