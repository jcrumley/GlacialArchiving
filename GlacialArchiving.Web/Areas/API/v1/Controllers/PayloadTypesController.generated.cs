...
{
    /// <summary>
    ///     Creates the interface via WebAPI for PayloadTypes
    /// </summary>
    public partial class PayloadTypesController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of PayloadTypes in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all PayloadTypes in the system that the logged in user can see</returns>
        public IEnumerable<PayloadType> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<PayloadType> collection = work.PayloadTypeRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<PayloadType> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the PayloadType record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public PayloadType Get(int id, [ModelBinder] LoggedInUser user)
        {
        ...
        partial void OnGetById(PayloadType obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a PayloadType record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] PayloadType obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.PayloadTypeRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(PayloadType obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(PayloadType obj, LoggedInUser user);
        
        /// <summary>
        ///     Deletes a PayloadType  record from the system
        /// </summary>
        ...
        public HttpResponseMessage Delete(int id, [ModelBinder] LoggedInUser user)
        {
            ...
            try
            {
                ...
                work.PayloadTypeRepository.Delete(id);
                work.Save();
