...
{
    /// <summary>
    ///     Creates the interface via WebAPI for Clients
    /// </summary>
    public partial class ClientsController : SecureApiController
    {
        /// <summary>
        ///     Query the listing of Clients in the system
        /// </summary>
        /// <param name="user">The current logged in user</param>
        /// <returns>Listing of all Clients in the system that the logged in user can see</returns>
        public IEnumerable<Client> Get([ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetAll", "Start of function");
            IQueryable<Client> collection = work.ClientRepository.All;
            this.OnGetAll(collection, user);
            this.log.Info("GetAll", "Returning data");
            return collection;
        }
        partial void OnGetAll(IQueryable<Client> collection, LoggedInUser user);
        
        /// <summary>
        ///     Returns the Client record specified by the id
        /// </summary>
        /// <param name="id">The id of the record to return</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public Client Get(int id, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("GetById", "Start of function");
            Client obj = work.ClientRepository.Find(id);
            this.OnGetById(obj, user);
            this.log.Info("GetById", "Returning data");
            return obj;
        }
        partial void OnGetById(Client obj, LoggedInUser user);
        
        /// <summary>
        ///     Creates or updates a Client record in the system
        /// </summary>
        /// <param name="obj">The object to create or update</param>
        /// <param name="user">The current logged in user</param>
        /// <returns></returns>
        public HttpResponseMessage Post([FromBody] Client obj, [ModelBinder] LoggedInUser user)
        {
            this.log.Info("Post", "Start of function");
            if (ModelState.IsValid)
            {
                this.OnBeforeSaveUpdate(obj, user);
                work.ClientRepository.InsertOrUpdate(obj, user.User);
                work.Save();
        ...
        partial void OnBeforeSaveUpdate(Client obj, LoggedInUser user);
        partial void OnAfterSaveUpdate(Client obj, LoggedInUser user);
        ...
        public HttpResponseMessage Delete(int id, [ModelBinder] LoggedInUser user)
        {
            ...
            try
            {
                ...
                work.ClientRepository.Delete(id);
                work.Save();
